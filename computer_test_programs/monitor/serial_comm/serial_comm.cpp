...
void  pollport(void *args) // threaded function to poll serial port
{
	...
	while (i < bufferlength && read_port->quit == 0) 
	{
		...
		if ((eventMask & EV_RXCHAR) == EV_RXCHAR)
		{
			...
			data2[i] |=(((short int)szBuff[0]) << 8) & (short int)0xFF00;
			merkki =  ((short int)szBuff[1]) & (short int)0x00FF;
			i++;
		}
	}
	...
	{
		fprintf(fptr, "%d\t%d\n", (short int)data[i], (short int)data2[i]);
	}
...
{
	HANDLE hSerial;
	hSerial = CreateFile(	L"\\\\.\\COM4",//COM5
